...
def test_unauthenticated_endpoints_rejected():
    ...
    assert (
        client.post(
            "/applications", json={"company_id": 1, "role_title": "Dev"}
        ).status_code
        == 401
    )
    ...
    assert (
        client.post("/applications/1/contacts", json={"name": "John"}).status_code
        == 401
    )
    ...
    assert (
        client.post("/contacts/1/outreach", json={"message_sent": "Hello"}).status_code
        == 401
    )
    assert (
        client.patch("/outreach/1", json={"response_received": True}).status_code == 401
    )
    ...
    assert (
        client.post(
            "/applications/1/interview-rounds",
            json={"scheduled_date": "2026-09-01T00:00:00Z"},
        ).status_code
        == 401
    )
    assert (
        client.patch("/interview-rounds/1", json={"outcome": "passed"}).status_code
        == 401
    )
