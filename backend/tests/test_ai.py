...
async def test_ai_prep_notes_flow(auth_headers):
    ...
    with patch("app.services.ai_service.AIService.generate_interview_prep", new_callable=AsyncMock) as mock_ai:
        mock_ai.return_value = mock_ai_output

        # Generate Prep Notes
        gen_resp = client.post(f"/applications/{app_id}/ai/prep-notes", headers=auth_headers)
        ...
        get_resp = client.get(f"/applications/{app_id}/ai/prep-notes", headers=auth_headers)
        assert get_resp.status_code == status.HTTP_200_OK
        assert get_resp.json()["id"] == gen_data["id"]

        # 5. Delete Prep Notes
        del_resp = client.delete(f"/applications/{app_id}/ai/prep-notes", headers=auth_headers)
        assert del_resp.status_code == status.HTTP_204_NO_CONTENT

        # 6. Verify 404 after deletion
        get_after_del = client.get(f"/applications/{app_id}/ai/prep-notes", headers=auth_headers)
